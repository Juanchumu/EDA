...
bool l_agregar (Lista lista, TipoElemento elemento) {
	if (l_es_llena(lista)) {return false;} //Controlo la lista llena
	int p;
...
bool l_borrar (Lista lista, int clave) {
	if (l_es_vacia(lista)) { return false; } // Controlo la lista vacia
	int q; 
	int p = lista->inicio; 
	bool borre = false;
	...
	while ((p != NULO) && (lista->cursor[p].siguiente != NULO)) {
		...
		if(lista->cursor[p].datos->clave == clave){
			...
			lista->libre = q;
			lista->cantidad--;
			p  = qant; //vuelvo a tomar el qant para revisar que
					   //no existan otras claves iguales
			borre = true;
		}else{
...
bool l_insertar (Lista lista, TipoElemento elemento, int pos) {
	if (l_es_llena(lista)) {return false;} //Control de lista llena
